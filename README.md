...
``` shell
$   npm install -g laravel-echo-server-hooks
```
...
``` shell
$   laravel-echo-server-hooks init
```
...
``` shell
$ laravel-echo-server-hooks client:add APP_ID
```
...
You can remove clients with `laravel-echo-server-hooks client:remove APP_ID`
...
``` shell
$ laravel-echo-server-hooks start
```
...
``` shell
$ laravel-echo-server-hooks stop
```
...
The working directory in which `laravel-echo-server` will look for the configuration file `laravel-echo-server.json` can be passed to the `start` command through the `--dir` parameter like so: `laravel-echo-server-hooks start --dir=/var/www/html/example.com/configuration`
